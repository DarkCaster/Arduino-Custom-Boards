...
:findMake
%DEBUG% Looking for make in current path
call :clearerrors
call :which make.exe
IF "%gotwhich%" NEQ "" (
  %DEBUG% make.exe already installed at %gotwhich%
  exit /b 123
) ELSE (
  %DEBUG% No make.exe in current path, trying %ScriptRoot%..\..\..\gnuwin32_make
  call :shorten %ScriptRoot%..\..\..\gnuwin32_make
  PATH %PATH%;!shortout!
)
...
call :which make.exe
IF "%gotwhich%" NEQ "" (
  %DEBUG% make.exe found at %gotwhich%
  exit /b 123
)
...
IF [%gotwhich%] NEQ [] (
  %DEBUG% avr-gcc already installed at %gotwhich%
  exit /b 123
) ELSE (
  %DEBUG% No avr-gcc in current path
)
...
:scandir
  ...
  FOR /F "tokens=*" %%f IN ('dir /b /x "%root%\Arduino*"') DO (
    ...
    if exist "!prg!\Hardware\tools\avr\bin\avr-gcc.exe" (
       if "%printed%" NEQ "Y" (
          %DEBUG% At least one Arduino install found.
          set printed=Y
       )
       %DEBUG% Looks like !prg! has version 
...
@echo no arduino installations was found!
exit /b 1
...
REM
%DEBUG% adding arduino bin and etc to PATH %bin%;%etc%
call :shorten "%bin%"
PATH %PATH%;!shortout!
call :shorten "%etc%"
PATH %PATH%;!shortout!
