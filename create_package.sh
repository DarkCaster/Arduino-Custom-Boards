...
if [[ ! -z $MSYSTEM ]]; then
  ...
  for test_dir in "$program_files_x86"/Arduino*/hardware/tools/avr/bin "$HOME/AppData/Local/Arduino"*/packages/arduino/tools/avr-gcc/*/bin; do
    [[ ! -d $test_dir ]] && continue
  ...
  # find arduino installation, and setup avr-gcc
  log "looking for avr-gcc from arduino installation"
  for test_dir in /usr/bin "$HOME"/arduino-*/hardware/tools/avr/bin "$HOME/.arduino"*/packages/arduino/tools/avr-gcc/*/bin; do
    [[ ! -d $test_dir ]] && continue
  ...
  [[ $arduino_found != true ]] && log "avr-gcc not found!" && false
  [[ -z $(which 2>/dev/null zip) ]] && log "zip (Info-zip) utility not found!" && false
  shautil=$(which 2>/dev/null sha256sum)
...
build_optiboot() {
  ...
  pushd 1>/dev/null "$temp_dir/optiboot_build"
...
build_optiboot 20000000L 115200 "atmega328 atmega328p atmega328pb"
build_optiboot 20000000L 57600 "atmega328 atmega328p atmega328pb"
build_optiboot 16000000L 250000 "atmega328 atmega328p atmega328pb atmega1284p atmega2560"
build_optiboot 16000000L 115200 "atmega328 atmega328p atmega328pb atmega1284p atmega2560"
build_optiboot 16000000L 57600 "atmega328 atmega328p atmega328pb atmega1284p atmega2560"
...
for entry in "$curdir/packages/"*.json; do
  [[ ! -f "$entry" ]] && break
